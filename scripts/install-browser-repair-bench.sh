#!/usr/bin/env bash
# Install the browser repair beside the current bench bundle and restart the shell.
set -euo pipefail

test "$(id -u)" = 0
staging=/var/tmp/pc1-browser-fix-20260905
previous=/var/marwanos/dev-shell/pc1-keyboard-20260905
target=/var/marwanos/dev-shell/pc1-browser-fix-20260905
override=/var/marwanos/dev-shell/marwanos-shell
backup="$override.before-browser-fix-20260905"
...
# The copied payload retains the root-owned Chromium sandbox and its SUID bit.
...
for attempt in $(seq 1 30); do
    ...
    if test -n "$new_pid"; then
        printf 'Browser repair activated; shell restarted as PID %s.\n' "$new_pid"
        exit 0
    fi
    sleep 0.5
done

printf 'The wrapper was installed, but the session did not start the repaired shell.\n' >&2
exit 1
