...
##
## Phase 1 deletes this file. The grid is populated from marwand's
...
## the source changes this file and nothing in the grid. `accent` stands in for
## `icon` for now: real icons arrive as a texture the daemon has cached, and the
## tile learns to load one asynchronously in Phase 1 M2. A flat colour is what a
## tile with no icon yet has to fall back to anyway, so it is not throwaway.
##
## Twelve entries, four across. Microsoft's 10-foot guidance caps traversal at six
## clicks edge to edge, so six columns is the ceiling; four leaves the tiles large
## enough to read a title on from three metres.
##
...
## whether the grid was showing baked data or something it had actually enumerated.
...
static func entries() -> Array:
	return [
		...
		{"id": "placeholder.08", "title": "Placeholder Eight", "subtitle": "Grid grows with the library", "accent": "#4A6B8A"},
		...
		{"id": "placeholder.12", "title": "Placeholder Twelve", "subtitle": "Last tile in the grid", "accent": "#6A5F7E"},
	]
