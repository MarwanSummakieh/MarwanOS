...
func _modal_open() -> bool:
	return _menu != null or _keyboard != null or _viewer != null or _properties != null \
		or _browser != null or _installer != null
...
func _paste_one(src: String, dest_dir: String, cut: bool) -> Dictionary:
	...
	if cut:
		...
		if err != OK:
			var skipped_before := _skipped_links
			err = _copy_any(src, dest, src_is_dir)
			if err == OK and _skipped_links != skipped_before:
				return {"error": "Copied %s, but kept the original because it contains links" % src_name}
			if err == OK:
