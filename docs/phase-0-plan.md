...
- [ ] **Fullscreen at native resolution is NOT met, and the cause is not the desk setup.** The shell was handed one surface covering *both* connected displays and laid itself out across the pair: the header's left end — the `MarwanOS` wordmark from `_build_header()` in `shell/src/shell_root.gd` — rendered on the laptop panel, while the right end of that same `HBoxContainer` (`Player 1  DualSense Wireless Controller`) rendered on the external display, with the grid's first column falling in the gap between the two panels. One row of one container, split across two screens, is not a Godot scaling failure: the scene is intact and `stretch/aspect="keep"` did exactly what it promises. It is the compositor handing the client an output layout spanning every connected display. The mechanism is cage's, not gamescope's: gamescope's DRM backend holds a single connector and a single CRTC and has no output-layout concept, while cage maximises its one view to the bounding box of *every* output and `extend` is its default mode. That is inference from mechanism rather than from pixel measurements — and it is contradicted by [ADR 0004](adr/0004-session-compositor-scaffold.md)'s record of plan A starting and rendering on 2026-08-03, so both may be true of different boots. Settle it with `journalctl -b -t marwanos-session -o cat` before writing ADR 0005. Note `-u greetd` will not find it: greetd creates a logind session, so the session's processes live in `session-cN.scope`
...
- [ ] **The karg itself is still unshipped**, held on the connector name: `video=` is a plain name match with no error path, so a wrong token is a silent no-op. Read the real name off the target first (see [dev-setup.md](dev-setup.md)), then build it onto a **second** stick with the current one left untouched — the command line lives inside a UKI and cannot be edited at boot
